...
% Material Damping, damping factor of the selected mode
zeta_0 = 0.01;
% this is the index of the mode you pick the damping of.  
% Note it also sets all the other modes as well, but we 
% only have direct choice of a single one.
idx_zeta = 1; 
...
%% BC loading face:
[f_nel, f_eltype, f_IEN] = parse_msh(msh, 'face6');
...
quad_rules = set_integration_rules( [eltype; f_eltype], ...
    'el10', struct('method', 'prod', 'points', 7) );
...
kappa_k = 2*zeta_0/omega_list(idx_zeta);
D = kappa_m*M + kappa_k*K;
D1 = D(freefree_range, freefree_range);

zeta_list = (kappa_m./omega_list + kappa_k*omega_list)/2;

if flags.plot_dampingFactors
    fig_damp = figure();
    ax_damp = axes('Parent', fig_damp);
    hold(ax_damp, 'on');
    xlabel(ax_damp,'Mode number');
    ylabel(ax_damp,'Damping factor');
    ax_damp.YScale = 'log';
    ax_damp.XScale = 'log';
    plot( ax_damp, 1:neq, zeta_list, '-o' );
    grid(ax_damp, 'on');
    plot( ax_damp, [1, neq], [1, 1], 'k-', 'LineWidth', 2);
